...
# Fix logsField and progressField at 3579
app = app.replace('[logsField]: [...logs, `[SYSTEM] 影片生成失敗，進行第 ${currentRetryCount + 1} 次重試...`],',
                  'videoLogsKeyframes: [...logs, `[SYSTEM] 影片生成失敗，進行第 ${currentRetryCount + 1} 次重試...`],')
app = app.replace('[progressField]: "50%"\n                         };',
                  'videoProgressKeyframes: "50%"\n                         };')
    ...
    for (const scene of activeProject.scenes) {
      ...
      try {
        ...
        while (true) {
          ...
          const p = JSON.parse(localStorage.getItem("toonflow_projects") || "[]").find((p) => p.id === activeProjectId);
          const s = p?.scenes.find((s) => s.id === scene.id);
          if (s?.videoUrl || s?.isForcePassed) break;
          if (s?.videoError || s?.videoErrorKeyframes || s?.videoErrorExt) {
...
app = app.replace('return (\n    <div className="min-h-screen bg-slate-900', missing_funcs + '\n  return (\n    <div className="min-h-screen bg-slate-900')
